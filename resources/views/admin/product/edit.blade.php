...
@section('content')
    <div class="container">
        ...
        <div class="form-container">
            <form id="designForm" action="{{ route('admin.updateProduct', $post->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                @if(!empty($oldImages))
                    <div class="input-group">
                        ...
                        <div class="preview-grid">
                            @foreach($oldImages as $p)
                                <div class="thumb">
                                    <img src="{{ asset($p) }}" alt="">
                                    <span style="text-align: center;display: block;padding: 10px 0">Xoá ảnh này</span>
                                    <label class="name" title="{{ $p }}">
                                        <input type="checkbox" name="remove[]" value="{{ $p }}" style="margin-right:6px">
                ...
                {{-- Thêm ảnh mới --}}
                <div class="input-group">
                    <label for="images">Thêm ảnh mới (nhiều ảnh)</label>
                ...
                <div class="input-group">
                    <label for="categories" class="form-label fw-bold text-dark">
                        <i class="fas fa-list"></i> Danh mục
                    </label>
                    <select name="category_id[]" id="categories" class="text-dark form-control select2 @error('category_id') is-invalid @enderror" multiple>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ in_array($category->id, explode(',', old('category_id', $post->category_id))) ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
...
@push('script')
    ...
    <script>
        ...
        input.addEventListener('change', (e) => {
            ...
            if (merged.length !== valid.length) alert('Một số ảnh không đúng định dạng hoặc > 5MB.');
            currentFiles = valid;
