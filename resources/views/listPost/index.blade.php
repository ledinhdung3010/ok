    ...
    <div class="content ">
                                                ...
                                                @foreach($post as $value)
                                                    <article class='mix portfolio_category_615 portfolio_category_69 ' style=''>
                                                        <div class="item_holder slow_zoom">
                                                            ...
                                                            <a itemprop="url" class="portfolio_link_class" title="{{$value->title}}" href="{{ route('detailPost', ['slug' => $value->slug]) }}"></a>
        ...
        $firstImage = $images[0]; // Lấy ảnh đầu tiên trong mảng
                                                        ...
                                                        <div class='portfolio_description ' style=background-color:#ffffff;'>
                                                            <h5  style="text-align: center" itemprop="name" class="portfolio_title entry_title"><a itemprop="url" href="{{route('detailPost', ['slug' => $value->slug])}}" target="_self" >{{$value->title}}</a></h5>
                                                        </div>
                                                    </article>
                                                @endforeach
